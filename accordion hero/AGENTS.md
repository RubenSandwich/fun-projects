...
(`node --experimental-strip-types --test`), and the pitch/sound/constants
...
pitch/sound/constants test-graph files (kept on relative `.ts` imports so the
...
- React 19 + Vite 8 + **TypeScript** (strict, `.tsx`/`.ts`; type-check with
  ...
  their owning module (`Direction`/`Preset` in `data/constants.ts`,
  `Song`/`Note`/`Difficulty` in `data/songs.ts`, `Detection` in `audio/pitch.ts`,
  `GameResult`/`GameNote` in `hooks/useGameEngine.ts`).
...
src/
  ...
  data/constants.ts       lanes, KEY_CODES, LANE_NOTES, timing, noteX(), and the
                          note-frequency PRESET store (localStorage)
  data/songs.ts           chart parser, buildSong, built-in songs, and the SONG
...
- **Push vs Pull** = bellows direction. **Push = tap the key**, **Pull = hold
  ...
  push vs pull. The full map lives in `LANE_NOTES` in `data/constants.ts`
...
- **Note-frequency presets** (`data/constants.ts`; keys `accordion-note-presets`,
  ...
  `NoteFreq`).
- **Song library** (`data/songs.ts`; key `accordion-user-songs`): `getSongs`
...
chart }` to `BUILTIN_DEFS` in `data/songs.ts` (chart in the `+N` / `-N`
