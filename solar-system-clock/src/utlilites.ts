...
export function logTimes(universeState): void {
	...
	const jsonData = {
		...
		// I want dates to be converted through toUTCString
		times: Object.keys(universeState.times)
			.filter((key) => universeState.times[key])
			.reduce((acc, key) => {
				const date = universeState.times[key];

				// planetRemoved is an array of dates
				if (Array.isArray(date)) {
					acc[key] = date.map((d) => d.toUTCString());
				} else {
