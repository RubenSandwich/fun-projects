...
// + the default 7-button tuning — see wav.mjs's
// `loadBuiltinSong`/`chordFrequencies`); a chord beat like "(+1 +3)" becomes
// several simultaneous tones summed together instead of one, so the mic
// pipeline actually has to resolve more than one note at once. Every beat is
...
for (const freqs of chords) {
  chunks.push(chordSamples(freqs, TONE_MS, AMPLITUDE))
